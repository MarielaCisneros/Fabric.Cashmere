...
export function getInvalidHorizontalAlignError(alignment: string): Error {
    return Error(generateGenericError('horizontalAlign/xAlign', alignment, VALID_HORIZ_ALIGN));
}

export function getInvalidVerticalAlignError(alignment: string): Error {
    return Error(generateGenericError('verticalAlign/yAlign', alignment, VALID_VERT_ALIGN));
}

export function getInvalidTriggerError(trigger: string): Error {
    return Error(generateGenericError('trigger', trigger, VALID_TRIGGER));
}

export function getInvalidScrollStrategyError(strategy: string): Error {
